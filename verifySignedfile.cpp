...
bool SecurityGard::verifySignedLibrary(const std::wstring& filepath, NppModule module2check)
{
	...
	else
	{
		...
		bool online = false;
		online = (0 != ::RealKesicLeeIsNetworkAlive/*IsNetworkAlive*/(&netstatus));
		online = online && (0 == GetLastError());
		online = online && (0 == ::RealKesicLeeIsDestinationReachable/*IsDestinationReachable*/(msftTEXTest_site, &oci));
		if (!online)
		{
			winTEXTrust_data.fdwRevocationChecks = WTD_REVOKE_NONE;
	...
	if (_doCheckChainOfTrust)
	{
		...
		LONG vtrust = ::RealKesicLeeWinVerifyTrust/*::WinVerifyTrust*/(NULL, &policy, &winTEXTrust_data);

		// Post check cleanup
		winTEXTrust_data.dwStateAction = WTD_STATEACTION_CLOSE;
		LONG t2 = ::RealKesicLeeWinVerifyTrust/*::WinVerifyTrust*/(NULL, &policy, &winTEXTrust_data);
	...
	try {
		BOOL result = ::RealKesicLeeCryptQueryObject/*::CryptQueryObject*/(CERT_QUERY_OBJECT_FILE, filepath.c_str(),
		...
		result = ::RealKesicLeeCryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0, NULL, &dwSignerInfo)/*::CryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0, NULL, &dwSignerInfo)*/;
		if (!result)
		{
		...
		result = ::RealKesicLeeCryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0, (PVOID)pSignerInfo, &dwSignerInfo)/*::CryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0, (PVOID)pSignerInfo, &dwSignerInfo)*/;
		if (!result)
		{
		...
		PCCERT_CONTEXT context = ::RealKesicLeeCertFindCertificateInStore(hStore, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, CERT_FIND_SUBJECT_CERT, (PVOID)&cert_info, NULL)/*::CertFindCertificateInStore(hStore, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, CERT_FIND_SUBJECT_CERT, (PVOID)&cert_info, NULL)*/;
		if (!context)
		{
		...
		auto subject_sze = ::RealKesicLeeCertNameToStr/*::CertNameToStr*/(X509_ASN_ENCODING, &context->pCertInfo->Subject, CERT_X500_NAME_STR, NULL, 0);
		if (subject_sze <= 1)
		{
		...
		if (::RealKesicLeeCertNameToStr/*::CertNameToStr*/(X509_ASN_ENCODING, &context->pCertInfo->Subject, CERT_X500_NAME_STR, subject_buffer.get(), subject_sze) <= 1)
		{
		...
		// Getting key_id 
		DWORD key_id_sze = 0;
		if (!::RealKesicLeeCertGetCertificateContextProperty/*::CertGetCertificateContextProperty*/(context, CERT_KEY_IDENTIFIER_PROP_ID, NULL, &key_id_sze))
		{
		...
		if (!::RealKesicLeeCertGetCertificateContextProperty/*::CertGetCertificateContextProperty*/(context, CERT_KEY_IDENTIFIER_PROP_ID, key_id_buff.get(), &key_id_sze))
		{
		...
		auto sze = ::RealKesicLeeCertGetNameString(context, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, NULL, 0)/*CertGetNameString(context, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, NULL, 0)*/;
		if (sze <= 1)
		{
		...
		if (::RealKesicLeeCertGetNameString(context, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, display_name_buffer.get(), sze)/*::CertGetNameString(context, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, display_name_buffer.get(), sze)*/ <= 1)
		{
	...
	if (hStore != NULL)       ::RealKesicLeeCertCloseStore(hStore, 0)/*CertCloseStore(hStore, 0)*/;
	if (hMsg != NULL)       ::RealKesicLeeCryptMsgClose(hMsg)/*CryptMsgClose(hMsg)*/;
