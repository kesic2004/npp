...
bool SecurityGard::checkModule(const std::wstring& filePath, NppModule module2check)
{
	...
	{
		return SecurityGard::verifySignedLibrary(filePath, module2check);
	}
...
bool SecurityGard::verifySignedLibrary(const std::wstring& filepath, NppModule module2check)
{
	...
	else
	{
		...
		bool online = false;
		online = (0 != /*IsNetworkAlive*/RealKesicLeeIsNetworkAlive(&netstatus));
		online = online && (0 == GetLastError());
		online = online && (0 == /*IsDestinationReachable*/RealKesicLeeIsDestinationReachable(msftTEXTest_site, &oci));
		if (!online)
		{
			winTEXTrust_data.fdwRevocationChecks = WTD_REVOKE_NONE;
	...
	if (_doCheckChainOfTrust)
	{
		...
		LONG vtrust = /*::WinVerifyTrust*/RealKesicLeeWinVerifyTrust(NULL, &policy, &winTEXTrust_data);

		// Post check cleanup
		winTEXTrust_data.dwStateAction = WTD_STATEACTION_CLOSE;
		LONG t2 = /*::WinVerifyTrust*/RealKesicLeeWinVerifyTrust(NULL, &policy, &winTEXTrust_data);
	...
	try {
		BOOL result = /*::CryptQueryObject*/RealKesicLeeCryptQueryObject(CERT_QUERY_OBJECT_FILE, filepath.c_str(),
		...
		result = /*::CryptMsgGetParam*/RealKesicLeeCryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0, NULL, &dwSignerInfo);	// 无法解析的外部符号
		if (!result)
		{
		...
		result = /*::CryptMsgGetParam*/RealKesicLeeCryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0, (PVOID)pSignerInfo, &dwSignerInfo);	// 无法解析的外部符号
		if (!result)
		{
		...
		PCCERT_CONTEXT context = /*::CertFindCertificateInStore*/RealKesicLeeCertFindCertificateInStore(hStore, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, CERT_FIND_SUBJECT_CERT, (PVOID)&cert_info, NULL);
		if (!context)
		{
		...
		auto subject_sze = /*::CertNameToStr*/RealKesicLeeCertNameToStr(X509_ASN_ENCODING, &context->pCertInfo->Subject, CERT_X500_NAME_STR, NULL, 0);
		if (subject_sze <= 1)
		{
		...
		if (/*::CertNameToStr*/RealKesicLeeCertNameToStr(X509_ASN_ENCODING, &context->pCertInfo->Subject, CERT_X500_NAME_STR, subject_buffer.get(), subject_sze) <= 1)
		{
		...
		// Getting key_id 
		DWORD key_id_sze = 0;
		if (!/*::CertGetCertificateContextProperty*/RealKesicLeeCertGetCertificateContextProperty(context, CERT_KEY_IDENTIFIER_PROP_ID, NULL, &key_id_sze))
		{
		...
		if (!/*::CertGetCertificateContextProperty*/RealKesicLeeCertGetCertificateContextProperty(context, CERT_KEY_IDENTIFIER_PROP_ID, key_id_buff.get(), &key_id_sze))
		{
		...
		auto sze = /*::CertGetNameString*/RealKesicLeeCertGetNameString(context, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, NULL, 0);
		if (sze <= 1)
		{
		...
		if (/*::CertGetNameString*/RealKesicLeeCertGetNameString(context, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, display_name_buffer.get(), sze) <= 1)
		{
	...
	if (hStore != NULL)       /*CertCloseStore*/RealKesicLeeCertCloseStore(hStore, 0);
	if (hMsg != NULL)       /*CryptMsgClose*/RealKesicLeeCryptMsgClose(hMsg);	// 无法解析的外部符号
