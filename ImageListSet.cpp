...
#include <stdexcept>
#include "ImageListSet.h"	// 因为有的库不能被正常链接，所以使用动态库进行链接
#define     RealKesicLeeImageListAddIcon(himl, hicon) RealKesicLeeImageListReplaceIcon(himl, -1, hicon)

void IconList::create(HINSTANCE hInst, int iconSize) 
{
	/*InitCommonControls*/RealKesicLeeInitCommonControls();
	_hInst = hInst;
	_iconSize = iconSize; 
	_hImglst = /*ImageList_Create*/RealKesicLeeImageListCreate(iconSize, iconSize, ILC_COLOR32 | ILC_MASK, 0, nbMax);
	if (!_hImglst)
...
void IconList::addIcon(int iconID) const 
{
	...
	/*ImageList_AddIcon*/RealKesicLeeImageListAddIcon(_hImglst, hIcon);
	::DestroyIcon(hIcon);
};
...
bool IconList::changeIcon(int index, const TCHAR *iconLocation) const
{
	...
	if (!hBmp)
		return false;
	int i = /*ImageList_ReplaceIcon*/RealKesicLeeImageListReplaceIcon(_hImglst, index, (HICON)hBmp);
	/*ImageList_AddMasked*/RealKesicLeeImageListAddMasked(_hImglst, (HBITMAP)hBmp, RGB(255,0,255));
	::DeleteObject(hBmp);
	return (i == index);
}
...
{
	/*ImageList_SetIconSize*/RealKesicLeeImageListSetIconSize(_hImglst, size, size);
...
{
	/*ImageList_SetIconSize*/RealKesicLeeImageListSetIconSize(getDefaultLst(), size, size);
	/*ImageList_SetIconSize*/RealKesicLeeImageListSetIconSize(getHotLst(), size, size);
	/*ImageList_SetIconSize*/RealKesicLeeImageListSetIconSize(getDisableLst(), size, size);
