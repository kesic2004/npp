...
{
	::RealKesicLeeInitCommonControls/*InitCommonControls*/();
	_hInst = hInst;
	_iconSize = iconSize; 
	_hImglst = ::RealKesicLeeImageListCreate/*ImageList_Create*/(iconSize, iconSize, ILC_COLOR32 | ILC_MASK, 0, nbMax);
	if (!_hImglst)
...
void IconList::addIcon(int iconID) const 
{
	...
	::RealKesicLeeImageListAddIcon/*ImageList_AddIcon*/(_hImglst, hIcon);
	::DestroyIcon(hIcon);
};
...
bool IconList::changeIcon(int index, const TCHAR *iconLocation) const
{
	...
	if (!hBmp)
		return false;
	int i = ::RealKesicLeeImageListReplaceIcon/*ImageList_ReplaceIcon*/(_hImglst, index, (HICON)hBmp);
	::RealKesicLeeImageListAddMasked/*ImageList_AddMasked*/(_hImglst, (HBITMAP)hBmp, RGB(255,0,255));
	::DeleteObject(hBmp);
	return (i == index);
}
...
{
	::RealKesicLeeImageListSetIconSize/*ImageList_SetIconSize*/(_hImglst, size, size);
...
{
	::RealKesicLeeImageListSetIconSize/*ImageList_SetIconSize*/(getDefaultLst(), size, size);
	::RealKesicLeeImageListSetIconSize/*ImageList_SetIconSize*/(getHotLst(), size, size);
	::RealKesicLeeImageListSetIconSize/*ImageList_SetIconSize*/(getDisableLst(), size, size);
