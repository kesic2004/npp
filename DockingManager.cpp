...
LRESULT DockingManager::runProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		...
		case WM_DESTROY:
		{
			...
			if (_hImageList != NULL)
			{
				/*::ImageList_Destroy*/RealKesicLeeImageListDestroy(_hImageList);
			}
...
void DockingManager::createDockableDlg(tTbData data, int iCont, bool isVisible)
{
	...
	if ((data.uMask & DWS_ICONTAB) && data.hIconTab != NULL)
	{
		...
		if (_hImageList == NULL)
		{
			...
			_hImageList = /*::ImageList_Create*/RealKesicLeeImageListCreate(iconDpiDynamicalSize,iconDpiDynamicalSize,ILC_COLOR8, 0, 0);
		}

		// add icon
		/*::ImageList_AddIcon*/RealKesicLeeImageListAddIcon(_hImageList, data.hIconTab);
