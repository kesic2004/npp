...
LRESULT DockingManager::runProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		...
		case WM_DESTROY:
		{
			...
			if (_hImageList != NULL)
			{
				::RealKesicLeeImageListDestroy/*::ImageList_Destroy*/(_hImageList);
			}
...
void DockingManager::createDockableDlg(tTbData data, int iCont, bool isVisible)
{
	...
	if ((data.uMask & DWS_ICONTAB) && data.hIconTab != NULL)
	{
		...
		if (_hImageList == NULL)
		{
			...
			_hImageList = ::RealKesicLeeImageListCreate/*::ImageList_Create*/(iconDpiDynamicalSize,iconDpiDynamicalSize,ILC_COLOR8, 0, 0);
		}

		// add icon
		::RealKesicLeeImageListAddIcon/*::ImageList_AddIcon*/(_hImageList, data.hIconTab);
