...
BOOL FileBrowser::setImageList(int root_clean_id, int root_dirty_id, int open_node_id, int closed_node_id, int leaf_id) 
{
	...
	if ((_hImaLst = ::RealKesicLeeImageListCreate/*ImageList_Create*/(CX_BITMAP, CY_BITMAP, ILC_COLOR32 | ILC_MASK, nbBitmaps, 0)) == NULL) 
		return FALSE;
	...
	if (hbmp == NULL)
		return FALSE;
	::RealKesicLeeImageListAddMasked/*ImageList_AddMasked*/(_hImaLst, hbmp, maskColour);
	DeleteObject(hbmp);
	...
	if (::RealKesicLeeImageListGetImageCount/*ImageList_GetImageCount*/(_hImaLst) < nbBitmaps)
		return FALSE;
