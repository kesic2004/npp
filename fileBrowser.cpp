...
BOOL FileBrowser::setImageList(int root_clean_id, int root_dirty_id, int open_node_id, int closed_node_id, int leaf_id) 
{
	...
	if ((_hImaLst = /*ImageList_Create*/RealKesicLeeImageListCreate(CX_BITMAP, CY_BITMAP, ILC_COLOR32 | ILC_MASK, nbBitmaps, 0)) == NULL)
		return FALSE;
	...
	if (hbmp == NULL)
		return FALSE;
	/*ImageList_AddMasked*/RealKesicLeeImageListAddMasked(_hImaLst, hbmp, maskColour);
	DeleteObject(hbmp);
	...
	if (/*ImageList_GetImageCount*/RealKesicLeeImageListGetImageCount(_hImaLst) < nbBitmaps)
		return FALSE;
