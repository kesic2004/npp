...
BOOL FunctionListPanel::setTreeViewImageList(int root_id, int node_id, int leaf_id)
{
	...
	if ((_hTreeViewImaLst = /*ImageList_Create*/RealKesicLeeImageListCreate(CX_BITMAP, CY_BITMAP, ILC_COLOR32 | ILC_MASK, nbBitmaps, 0)) == NULL)
		return FALSE;
	...
	if (hbmp == NULL)
		return FALSE;
	/*ImageList_AddMasked*/RealKesicLeeImageListAddMasked(_hTreeViewImaLst, hbmp, maskColour);
	DeleteObject(hbmp);
	...
	if (/*ImageList_GetImageCount*/RealKesicLeeImageListGetImageCount(_hTreeViewImaLst) < nbBitmaps)
		return FALSE;
