...
BOOL FunctionListPanel::setTreeViewImageList(int root_id, int node_id, int leaf_id)
{
	...
	if ((_hTreeViewImaLst = ::RealKesicLeeImageListCreate/*ImageList_Create*/(CX_BITMAP, CY_BITMAP, ILC_COLOR32 | ILC_MASK, nbBitmaps, 0)) == NULL)
		return FALSE;
	...
	if (hbmp == NULL)
		return FALSE;
	::RealKesicLeeImageListAddMasked/*ImageList_AddMasked*/(_hTreeViewImaLst, hbmp, maskColour);
	DeleteObject(hbmp);
	...
	if (::RealKesicLeeImageListGetImageCount/*ImageList_GetImageCount*/(_hTreeViewImaLst) < nbBitmaps)
		return FALSE;
