		...
		case WM_MOUSEMOVE:
		{
			...
			else if (_bCapTTHover == FALSE)
			{
				...
				if ((_bCaptionTT == TRUE) || (_hoverMPos == posClose))
				{
					...
					_bCapTTHover = /*_TrackMouseEvent*/RealKesicLeeTrackMouseEvent(&tme);
				}
			}
		...
		case WM_MOUSEMOVE:
		{
            ...
            else
            {
				...
				if ((_bTabTTHover == FALSE) && (iItem != iItemSel))
				{
					...
					tme.dwFlags = TME_LEAVE | TME_HOVER;
					tme.dwHoverTime = 1000;
					_bTabTTHover = /*_TrackMouseEvent*/RealKesicLeeTrackMouseEvent(&tme);
				}
				else
				{
					if (iItem == iItemSel)
...
void DockingCont::drawTabItem(DRAWITEMSTRUCT *pDrawItemStruct)
{
	...
	if (((tTbData*)tcItem.lParam)->uMask & DWS_ICONTAB)
	{
		...
		if ((hImageList != NULL) && (iPosImage >= 0))
		{
			...
			/*ImageList_GetImageInfo*/RealKesicLeeImageListGetImageInfo(hImageList, iPosImage, &info);

			int iconDpiDynamicalY = NppParameters::getInstance()->_dpiManager.scaleY(7);
			/*::ImageList_Draw*/RealKesicLeeImageListDraw(hImageList, iPosImage, hDc, rc.left + 3, iconDpiDynamicalY, ILD_NORMAL);
