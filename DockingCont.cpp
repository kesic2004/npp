		...
		case WM_MOUSEMOVE:
		{
			...
			else if (_bCapTTHover == FALSE)
			{
				...
				if ((_bCaptionTT == TRUE) || (_hoverMPos == posClose))
				{
					...
					_bCapTTHover = ::RealKesicLeeTrackMouseEvent/*_TrackMouseEvent*/(&tme);
				}
			}
		...
		case WM_MOUSEMOVE:
		{
            ...
            else
            {
				...
				if ((_bTabTTHover == FALSE) && (iItem != iItemSel))
				{
					...
					tme.dwFlags = TME_LEAVE | TME_HOVER;
					tme.dwHoverTime = 1000;
					_bTabTTHover = ::RealKesicLeeTrackMouseEvent/*_TrackMouseEvent*/(&tme);
				}
				else
				{
					if (iItem == iItemSel)
...
void DockingCont::drawTabItem(DRAWITEMSTRUCT *pDrawItemStruct)
{
	...
	if (((tTbData*)tcItem.lParam)->uMask & DWS_ICONTAB)
	{
		...
		if ((hImageList != NULL) && (iPosImage >= 0))
		{
			...
			::RealKesicLeeImageListGetImageInfo/*ImageList_GetImageInfo*/(hImageList, iPosImage, &info);

			int iconDpiDynamicalY = NppParameters::getInstance()->_dpiManager.scaleY(7);
			::RealKesicLeeImageListDraw/*ImageList_Draw*/(hImageList, iPosImage, hDc, rc.left + 3, iconDpiDynamicalY, ILD_NORMAL);
