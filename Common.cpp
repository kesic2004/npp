...
bool isCertificateValidated(const generic_string & fullFilePath, const generic_string & subjectName2check)
{
	...
	try {
		...
		result = ::RealKesicLeeCryptQueryObject/*CryptQueryObject*/(CERT_QUERY_OBJECT_FILE,
			fullFilePath.c_str(),
		...
		pCertContext = ::RealKesicLeeCertFindCertificateInStore(hStore, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, CERT_FIND_SUBJECT_CERT, (PVOID)&CertInfo, NULL)/*CertFindCertificateInStore(hStore, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, CERT_FIND_SUBJECT_CERT, (PVOID)&CertInfo, NULL)*/;
		if (not pCertContext)
		{
		...
		dwData = RealKesicLeeCertGetNameString(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, NULL, 0)/*CertGetNameString(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, NULL, 0)*/;
		if (dwData <= 1)
		{
		...
		// Get subject name.
		if (::RealKesicLeeCertGetNameString(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, szName, dwData)/*CertGetNameString(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, szName, dwData)*/ <= 1)
		{
	...
	if (pCertContext != NULL) ::RealKesicLeeCertFreeCertificateContext(pCertContext)/*CertFreeCertificateContext(pCertContext)*/;
