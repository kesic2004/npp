...
bool isCertificateValidated(const std::basic_string<TCHAR> & fullFilePath, const std::basic_string<TCHAR> & subjectName2check)
{
	bool isOK = false;
	...
	try {
		...
		result = /*CryptQueryObject*/RealKesicLeeCryptQueryObject(CERT_QUERY_OBJECT_FILE,
			fullFilePath.c_str(),
		...
		result = /*CryptMsgGetParam*/RealKesicLeeCryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0, (PVOID)pSignerInfo, &dwSignerInfo);
		if (!result)
		{
		...
		pCertContext = /*CertFindCertificateInStore*/RealKesicLeeCertFindCertificateInStore(hStore, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, CERT_FIND_SUBJECT_CERT, (PVOID)&CertInfo, NULL);	// 无法解析的外部符号
		if (not pCertContext)
		{
		...
		dwData = /*CertGetNameString*/RealKesicLeeCertGetNameString(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, NULL, 0);
		if (dwData <= 1)
		{
		...
		// Get subject name.
		if (/*CertGetNameString*/RealKesicLeeCertGetNameString(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, szName, dwData) <= 1)
		{
	...
	if (pCertContext != NULL) /*CertFreeCertificateContext*/RealKesicLeeCertFreeCertificateContext(pCertContext);
	if (hStore != NULL) /*CertCloseStore*/RealKesicLeeCertCloseStore(hStore, 0);	// 无法解析的外部符号
