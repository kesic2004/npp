...
void Version::setVersionFrom(const generic_string& filePath)
{
	...
	{
		DWORD handle = 0;
		DWORD bufferSize = /*::GetFileVersionInfoSize*/RealKesicLeeGetFileVersionInfoSize(filePath.c_str(), &handle);

		if (bufferSize <= 0)
			return;

		unsigned char* buffer = new unsigned char[bufferSize];
		/*::GetFileVersionInfo*/RealKesicLeeGetFileVersionInfo(filePath.c_str(), handle, bufferSize, buffer);

		VS_FIXEDFILEINFO* lpFileInfo = nullptr;
		UINT cbFileInfo = 0;
		/*VerQueryValue*/RealKesicLeeVerQueryValue(buffer, TEXT("\\"), reinterpret_cast<LPVOID*>(&lpFileInfo), &cbFileInfo);
		if (cbFileInfo)
		{
